...
# Удаляем папку сессий (несколько попыток для гарантии)
echo "🗑️  Удаление .wwebjs_auth/..."
if [ -d ".wwebjs_auth" ]; then
    # Первая попытка
    rm -rf .wwebjs_auth/ 2>/dev/null
    # Вторая попытка с sudo (если нужны права)
    sudo rm -rf .wwebjs_auth/ 2>/dev/null
    # Третья попытка - изменить права и удалить
    chmod -R 777 .wwebjs_auth/ 2>/dev/null
    rm -rf .wwebjs_auth/ 2>/dev/null
    
    if [ -d ".wwebjs_auth" ]; then
        echo "❌ ОШИБКА: Не удалось удалить .wwebjs_auth/"
        echo "Попробуйте вручную: sudo rm -rf .wwebjs_auth/"
    else
        echo "✅ Папка удалена"
    fi
else
...
# Поиск и удаление всех возможных копий
echo "🔍 Поиск других копий .wwebjs_auth..."
find . -name ".wwebjs_auth" -type d -exec rm -rf {} + 2>/dev/null
echo "✅ Поиск завершён"

# Удаляем все архивы (несколько попыток)
echo "🗑️  Удаление всех архивов WhatsApp..."
rm -f whatsapp_auth_*.tar.gz 2>/dev/null
rm -f ./whatsapp_auth_*.tar.gz 2>/dev/null
sudo rm -f whatsapp_auth_*.tar.gz 2>/dev/null

# Поиск архивов в других местах
find . -name "whatsapp_auth_*.tar.gz" -type f -delete 2>/dev/null

if ls whatsapp_auth_*.tar.gz 1> /dev/null 2>&1; then
    echo "❌ ОШИБКА: Архивы всё ещё существуют"
